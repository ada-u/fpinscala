...
object SimpleStreamTransducer {
    ...
    def processFile[A, B](f: java.io.File,
      ...
      def go(ss: Iterator[String], cur: Process[String, A], acc: B): B =
        cur match {
          case Halt() => acc
          case Await(receive) =>
            val next = if (ss.hasNext) receive(Some(ss.next())) else receive(None)
            go(ss, next, acc)
  ...
  object NonComposableProcessingWithIO {
    ...
    def lineGt40k_3(filename: String): IO[Boolean] = {
      import fpinscala.streamingio.SimpleStreamTransducer.Process._
    ...
    def runLineGt40k: Boolean = {
      ...
      val app: IO[Boolean] = NonComposableProcessingWithIO.lineGt40k_3("/Users/ada-u/.tmux.conf")
